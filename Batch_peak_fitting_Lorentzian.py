...
for filename in glob.glob(os.path.join(path, '*.csv')):     
    if basename(filename)[-5] == 'd':
        ...
        try:
            ...
            amp3 = popt[7]
            wid3 = popt[8]            
            curve1 = amp1 * wid1**2 / ((Qlist-ctr1)**2 + wid1**2)
            curve2 = amp2 * wid2**2 / ((Qlist-ctr2)**2 + wid2**2)
            curve3 = amp3 * wid3**2 / ((Qlist-ctr3)**2 + wid3**2)
